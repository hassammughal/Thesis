...
//	.AddAttribute ("PacketSizes", "The size of packets sent in on states",
//	               UintegerValue (256),
//	               MakeUintegerAccessor (&OnOffApplication::m_pktSizes),
//	               MakeUintegerChecker<uint32_t> (1))
...
//      uint32_t bit = m_pktSizes * 8;
//      NS_LOG_LOGIC ("Bit = " << bit);
...
void OnOffApplication::SendPacket ()
{
  ...
  NS_LOG_INFO ("m_sent: " << m_sent[nodeID]);
  SeqTsHeader seqTs;
  seqTs.SetSeq (m_sent[nodeID]);
  NS_LOG_DEBUG("Packet Size: " << m_pktSize);
  Ptr<Packet> packet = Create<Packet> (m_pktSize-(8+4));
  m_txTrace (packet);
  ...
  if (InetSocketAddress::IsMatchingType (m_peer))
    {
      ...
      NS_LOG_INFO ("At time " << Simulator::Now ().GetSeconds ()
                   ...
                   << " total Tx " << m_totBytes << " bytes and Sequence Number: " << m_sent[nodeID]);
