...
public:
	...
	std::uint16_t getDeadline(){
		return m_deadline;
	}
	std::uint16_t getTaskSize(){
		return m_task_size;
	}
	std::uint16_t getTaskId(){
		return m_task_id;
	}
...
using deadlinePriorityQueue = std::priority_queue<UserTask, std::vector<UserTask>, decltype(cmp_deadline)>;
deadlinePriorityQueue orderedQueue(cmp_deadline);
...
template <typename T>
void RoutingExperiment::PopulateQueue(T &userTaskQueue){
	srand((int) time(NULL));
	...
	while (i < 5){
		...
		userTaskQueue.push(UserTask(dl,ds,i));
		i++;
...
RoutingExperiment::StartTaskGeneration()
{
	...
	PopulateQueue<deadlinePriorityQueue>(orderedQueue);
	GenerateTasks();
}
...
RoutingExperiment::GenerateTasks()
{
	...
	auto task = orderedQueue.top();
	double deadline = task.getDeadline();
	NS_LOG_DEBUG("Task ID: " << task.getTaskId() << ", deadline: " << deadline << "s");
	double dataSize = task.getTaskSize();
